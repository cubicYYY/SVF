...
PointerAnalysis::PointerAnalysis(SVFIR* p, PTATY ty, bool alias_check)
    : svfMod(nullptr), ptaTy(ty), stat(nullptr), ptaCallGraph(nullptr), callGraphSCC(nullptr), icfg(nullptr),
      chgraph(nullptr)
{
    pag = p;
...
void PointerAnalysis::initialize()
{
    ...
    // dump callgraph
    if (Options::CallGraphDotGraph()) getPTACallGraph()->dump("callgraph_initial");
}
...
void PointerAnalysis::finalize()
{
    ...
    if (Options::CallGraphDotGraph()) getPTACallGraph()->dump("callgraph_final");
